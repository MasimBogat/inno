$(document).ready(function() {
    ...
    myFrom.on('submit', function(e) {
        ...
        formData.append("message", $('textarea[name="t_message"]').val());
        if ($('body').hasClass('contact_form')) {
            let mailto = $('.info_box').find('li').first().find('a').text();
    ...
    companyTriggerForm.on('click',function(e) {
        ...
        if ( formWrap.hasClass('_show') ) {
            ...
            let formPos = formWrap.offset().top;
            $("html, body").animate({
                scrollTop: formPos
            }, 1000);
        }
